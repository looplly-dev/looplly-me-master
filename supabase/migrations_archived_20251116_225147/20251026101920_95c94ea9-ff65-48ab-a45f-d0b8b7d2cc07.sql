-- Update reset_user_journey to accept caller_user_id parameter
-- This fixes the issue where auth.uid() returns NULL in service role context

DROP FUNCTION IF EXISTS public.reset_user_journey(uuid, journey_stage);
...
BEGIN
  ...
  IF NOT has_role(p_caller_user_id, 'tester') AND NOT has_role(p_caller_user_id, 'admin') THEN
  ...
  CASE p_stage
    ...
    WHEN 'otp_verified' THEN
      ...
      -- UPDATE profiles SET is_verified = true WHERE user_id = p_target_user_id;  -- Column doesn't exist
