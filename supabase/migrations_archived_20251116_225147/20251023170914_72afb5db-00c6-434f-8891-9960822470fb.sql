...
-- Create reset_user_journey function for simulator
CREATE OR REPLACE FUNCTION reset_user_journey(
...
BEGIN
  ...
  -- Security: Only testers/admins can reset
  IF NOT has_role(auth.uid(), 'tester') AND NOT has_role(auth.uid(), 'admin') THEN
  ...
  CASE p_stage
    ...
    WHEN 'otp_verified' THEN
      PERFORM reset_user_journey(p_target_user_id, 'fresh_signup');
    ...
    WHEN 'basic_profile' THEN
      PERFORM reset_user_journey(p_target_user_id, 'otp_verified');
      UPDATE profiles SET
    ...
    WHEN 'full_profile' THEN
      PERFORM reset_user_journey(p_target_user_id, 'basic_profile');
      UPDATE profiles SET
        sec = 'C1',
    ...
    WHEN 'first_survey' THEN
      PERFORM reset_user_journey(p_target_user_id, 'full_profile');
    ...
    WHEN 'established_user' THEN
      PERFORM reset_user_journey(p_target_user_id, 'first_survey');
