-- Update reset_user_journey to support show_ui flag for basic_profile stage
...
BEGIN
  ...
  -- Store original test user identity BEFORE any resets
  ...
  CASE p_stage
    ...
    WHEN 'basic_profile' THEN
      -- Reset to fresh signup (preserves test user identity)
      PERFORM reset_user_journey(p_caller_user_id, p_target_user_id, 'fresh_signup');
      
      -- DON'T seed any data - let the registration form handle it
      -- The fresh_signup reset already preserves: first_name, last_name, mobile, country_code
      
      v_result = jsonb_build_object(
        'stage', 'basic_profile', 
        'description', 'Ready to complete Level 1 registration (Name/Mobile pre-filled)',
        'show_ui', 'registration_form'
      );
