...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[REGISTER] Using sync bcrypt for hashing');
    const salt = bcrypt.genSaltSync(8);
    const passwordHash = bcrypt.hashSync(password, salt);
    ...
    // Note: Skipping auth.users creation for now. The login function will create it when needed.
    // This allows registration to complete without auth service dependencies.
    
    // Insert into profiles table using stored function (bypasses PostgREST schema cache)
    const { data, error } = await supabase
      .rpc('insert_profile', {
        ...
        p_profile_level: 1,
        p_profile_completeness_score: 40
      });
